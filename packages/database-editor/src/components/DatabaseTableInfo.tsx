...
export const DatabaseTableInfo = ({ context, database }: { context: DatabaseEditorContext; database: string }) => {
  ...
  const databaseInfoQueryKey = useMemo(() => genQueryKey('datbaseInfo', tableContext), [tableContext]);

  const tables = useQuery({
    queryKey: databaseInfoQueryKey,
    queryFn: async () => {
  ...
  return <div className='table-info'>{validResponse() ? <TableData data={tables.data!} /> : 'no data'}</div>;
};
