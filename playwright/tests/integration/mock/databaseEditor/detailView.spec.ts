...
test('inputs', async () => {
  await editor.main.table.row(0).locator.click();
  await expect(editor.detailView.collapsibles).toHaveCount(3);

  const generalInputs = editor.detailView.collapsibles.nth(0).locator('.ui-field');
  const propertiesInputs = editor.detailView.collapsibles.nth(1).locator('.ui-field');
  const additionalInputs = editor.detailView.collapsibles.nth(2).locator('.ui-table-row');
  ...
  await editor.detailView.collapsibles.nth(2).click();
  await expect(additionalInputs).toHaveCount(5);
});

test('switch driver', async () => {
  await editor.main.table.row(0).locator.click();
  const driver = editor.detailView.locator.getByRole('combobox');
  ...
  await expect(editor.detailView.collapsibles.nth(1).locator('.ui-field')).toHaveCount(3);
});

test('title', async () => {
  await expect(editor.detailView.title).toHaveText('Connection Properties');
